// app.js
// Lógica principal del Tablero de Aportes y Presupuestos de Socios
...
async function loadState() {
  ...
  try {
    // Obtener transacciones y presupuestos en paralelo
  ...
  // Cargar opciones en el select de transacciones
...
function loadBudgetSelectOptions() {
  ...
  // Agrupar por fase para que se vea ordenado
  const phases = {};
...
function renderTransactions() {
  ...
  filtered.forEach(tx => {
    ...
    const budgetConceptHtml = associatedBudget 
      ? `<br><span style="font-size:0.75rem;color:var(--success-light)">📌 ${associatedBudget.concept}</span>` 
      : "";
...
function renderBudgets() {
  ...
  if (filtered.length === 0) {
    const noRecordsHtml = `<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:30px;">No se encontraron ítems de presupuesto cargados.</td></tr>`;
    tableBody.innerHTML = noRecordsHtml;
...
async function saveBudget(event) {
  ...
  try {
    const bgData = { concept, phase, currency, amount, date };
