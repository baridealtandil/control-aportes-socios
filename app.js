// app.js
// Lógica principal del Tablero de Aportes de Socios
...
async function loadDolarBlue() {
  ...
  try {
    ...
    widgetValue.innerHTML = `Promedio: <strong>$${promedio}</strong> <span style="font-size:0.75rem;color:var(--text-muted)">(${state.dolarBlue.fecha} hs - C: $${data.compra} / V: $${data.venta})</span>`;
  } catch (e) {
...
async function loadState() {
  ...
  try {
    const transactions = await window.AppStorage.getData();
    ...
    alert("No se pudo conectar con el servidor. Se muestran datos vacíos.");
  } finally {
...
// 4. CALCULAR Y RENDERIZAR DASHBOARD
    ...
    card.innerHTML = `
      ...
      <div class="partner-metrics">
        ...
        <div class="metric-row">
          ...
          <span class="metric-val ${isCompleted ? 'completed' : 'remaining'}">${isCompleted ? "Completado" : formatCurrency(remaining, "USD")}</span>
        </div>
      </div>
...
function renderTransactions() {
  ...
  if (filtered.length === 0) {
    const noRecordsHtml = `<tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:30px;">No se encontraron registros de aportes.</td></tr>`;
  ...
  filtered.forEach(tx => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="widget-pill btn-secondary" style="padding:4px 8px;font-size:0.75rem;">${tx.phase || "Fase 1"}</span></td>
      <td>${tx.concept}</td>
    ...
    mobileCard.innerHTML = `
      ...
      <div class="mobile-tx-concept">
        <strong>${tx.phase || "Fase 1"}:</strong> ${tx.concept}
      </div>
...
// 7. GESTIONAR ROL DE ADMINISTRADOR EN PANTALLA
...
// 8. ACCIONES DE TRANSACCIONES (AGREGAR / EDITAR / ELIMINAR)
async function saveTransaction(event) {
  ...
  const phase = document.getElementById("tx-phase").value;
...
window.editTransaction = function(id) {
  ...
  document.getElementById("tx-phase").value = tx.phase || "Fase 1: Alquiler e Infraestructura";
...
// 9. GESTIONAR PIN DE ADMIN DESDE LA WEB (CAMBIAR PIN)
...
// 11. INICIO DE SESIÓN DE ADMINISTRADOR
...
// 13. REGISTRAR LOS LISTENERS DE EVENTOS
